...
if (isset($_POST['btnAdd'])) {
       ...
       if (!empty($year) && !empty($title)&& !empty($description)) {
                ...
                $sql_query = "INSERT INTO grahanalu_tab (year,title,description)VALUES('$year','$title','$description')";
                $db->sql($sql_query);
                $result = $db->getResult();
                if (!empty($result)) {
                    $result = 0;
                ...
                if ($result == 1) {
                    $sql = "SELECT id FROM grahanalu_tab ORDER BY id DESC LIMIT 1";
                    $db->sql($sql);
                    $res = $db->getResult();
                    $grahanalu_tab_id = $res[0]['id'];
                    for ($i = 0; $i < count($_POST['sub_title']); $i++) {
                        ...
                        $sql = "INSERT INTO grahanalu_tab_variant (grahanalu_tab_id,sub_title,sub_description) VALUES('$grahanalu_tab_id','$sub_title','$sub_description')";
                        $db->sql($sql);
                    ...
                    $error['add_poojalu_tab'] = "<section class='content-header'>
                                                    <span class='label label-success'>Grahanalu Added Successfully</span> </section>";
                } else {
...
<section class="content-header">
    <h1>Add Grahanalu<small><a href='grahanalu.php'> <i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Grahanalu</a></small></h1>
