...
if (isset($_POST['btnEdit'])) {
   ...
   if (!empty($year) && !empty($title) && !empty($description))
    {
        $sql_query = "UPDATE grahanalu_tab SET year='$year',title='$title',description='$description' WHERE id =  $ID";
			...
			if (!empty($update_result)) {
                ...
                if ($update_result == 1)
                {
                    for ($i = 0; $i < count($_POST['sub_title']); $i++) {
                        $grahanalu_tab_id = $db->escapeString(($_POST['grahanalu_tab_variant_id'][$i]));
                        $sub_title = $db->escapeString(($_POST['sub_title'][$i]));
                        $sub_description = $db->escapeString(($_POST['sub_description'][$i]));
                        $sql = "UPDATE grahanalu_tab_variant SET sub_title='$sub_title',sub_description='$sub_description' WHERE id =$grahanalu_tab_id";
                        $db->sql($sql);
                    ...
                    if (
                        ...
                        for ($i = 0; $i < count($_POST['insert_sub_title']); $i++) {
                            ...
                            if (!empty($sub_title) || !empty($sub_description)) {
                                $sql = "INSERT INTO grahanalu_tab_variant (grahanalu_tab_id,sub_title,sub_description) VALUES('$ID','$sub_title','$sub_description')";
                                $db->sql($sql);
    
                            }
                        }
                    }
                        $error['update_panchangam'] = " <section class='content-header'><span class='label label-success'>Grahanalu  updated Successfully</span></section>";
                } else {
...
$sql_query = "SELECT * FROM grahanalu_tab WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();

$sql_query = "SELECT * FROM grahanalu_tab_variant WHERE grahanalu_tab_id =" . $ID;
$db->sql($sql_query);
$resslot = $db->getResult();

if (isset($_POST['btnCancel'])) { ?>
	<script>
		window.location.href = "grahanalu.php";
	</script>
<?php } ?>
<section class="content-header">
	<h1>
		Edit Grahanalu<small><a href='grahanalu.php'><i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Grahanalu</a></small></h1>
	...
	<div class="row">
		<div class="col-md-10">
			...
			<div class="box box-primary">
				...
				<!-- /.box-header -->
				<!-- form start -->
				<form id="edit_grahanalu_tab_form" method="post" enctype="multipart/form-data">
					<div class="box-body">
						   <div class="row">
						   ...
						   <div id="variations">
							...
							foreach ($resslot as $row) {
								...
								<div id="packate_div">
									<div class="row">
									    <input type="hidden" class="form-control" name="grahanalu_tab_variant_id[]" id="grahanalu_tab_variant_id" value='<?= $row['id']; ?>' />
									    <div class="col-md-4">
    ...
    $(document).on('click', '.remove_variation', function() {
        if ($(this).data('id') == 'data_delete') {
            if (confirm('Are you sure? Want to delete this row')) {
                var id = $(this).closest('div.row').find("input[id='grahanalu_tab_variant_id']").val();
                $.ajax({
