...
if (isset($_POST['btnAdd'])) {
       ...
       if (!empty($title)&& !empty($description)) {
                ...
                $sql_query = "INSERT INTO navarathnalu_tab (title,description)VALUES('$title','$description')";
                $db->sql($sql_query);
                $result = $db->getResult();
                if (!empty($result)) {
                    $result = 0;
                ...
                if ($result == 1) {
                    $sql = "SELECT id FROM navarathnalu_tab ORDER BY id DESC LIMIT 1";
                    $db->sql($sql);
                    $res = $db->getResult();
                    $navarathnalu_tab_id = $res[0]['id'];
                    for ($i = 0; $i < count($_POST['sub_title']); $i++) {
                        ...
                        $sql = "INSERT INTO navarathnalu_tab_variant (navarathnalu_tab_id,sub_title,sub_description) VALUES('$navarathnalu_tab_id','$sub_title','$sub_description')";
                        $db->sql($sql);
                    ...
                    $error['add_poojalu_tab'] = "<section class='content-header'>
                                                    <span class='label label-success'>Navarathnalu Added Successfully</span> </section>";
                } else {
...
<section class="content-header">
    <h1>Add Navarathnalu<small><a href='navarathnalu.php'> <i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Navarathnalu</a></small></h1>
