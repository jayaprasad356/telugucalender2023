   ...
   if (!empty($name) && !empty($description) && !empty($location))
    {
		if ($_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0 && !empty($_FILES['image'])) {
			...
			$upload_image = 'upload/temples/' . $filename;
			$sql = "UPDATE temples SET `image`='" . $upload_image . "' WHERE `id`=" . $ID;
			$db->sql($sql);
		}
             $sql_query = "UPDATE temples SET name='$name',description='$description',location='$location' WHERE id =  $ID";
			 $db->sql($sql_query);
			 $res = $db->getResult();
			...
			$error['update_temple'] = " <section class='content-header'><span class='label label-success'>Temple updated Successfully</span></section>";
			} else {
...
$sql_query = "SELECT * FROM temples WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();

if (isset($_POST['btnCancel'])) { ?>
	<script>
		window.location.href = "temples.php";
	</script>
<?php } ?>
<section class="content-header">
	<h1>
		Edit Temples<small><a href='temples.php'><i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Temples</a></small></h1>
